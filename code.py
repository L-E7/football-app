...
def calculate_player_stats(matches):
    ...
    for m in matches:
        ...
        for p in players_in_match:
            if p not in stats:
                stats[p] = {'Played': 0, 'Wins': 0, 'Draws': 0, 'Losses': 0, 'Goals': 0, 'This is an excellent final piece of debugging. The "error that disappears" is a classic sign of a problem within a `st.rerun()` block in Streamlit, especially when dealing with session state.

Let's analyze the situation and provide the definitive fix.

### The Problem: Why the Error Disappears

1.  You click **"Finish & Calculate Match"**.
2.  The code calculates the next match, updates `st.session_state.tournament['current_match']`, and resets other state variables (`g1`, `g2`, etc.).
3.  It then calls `st.rerun()`.
4.  The script starts again from the top. However, for a brief moment, some of the *old* UI elements might still be trying to render with the *new* state, causing a temporary conflict (like trying to access a player from the previous match that no longer exists in the `current_match` state).
5.  This conflict throws an error.
6.  Streamlit's auto-correction then does a *second* rerun, which fully stabilizes the app with the new state, and the error vanishes. But the initial crash prevents the flow from feeling smooth.

The root cause is that the reset of the score inputs (`st.session_state.g1 = 0`) happens *before* the `st.rerun()` that processes the *next* match. The correct place to reset them is when a **new tournament starts** or when a **new match is set up**.

### The Solution: Smarter State Resets

We will make two small but crucial changes to the logic:

1.  **Reset Scores at the Start:** When a new tournament is created, we will initialize `st.session_state.g1` and `st.session_state.g2` to 0. This is already in the code and is correct.
2.  **Reset Scores Before Next Match:** When you click "Finish & Calculate Match," we will perform all calculations for the *next* match and then call `st.rerun()`. The score inputs for the new match will automatically use the default values from the reset session state variables. The key is to remove the reset from *within* the button logic and let the initialization handle it.

This ensures a clean, error-free transition between matches.

---

### Full and Final `code.py` (with Transition Fix)

Here is the complete, final version of the code. I have adjusted the state management within the "Finish & Calculate Match" button to be more robust and prevent the temporary error.
...
<style>
    /* ... (CSS is unchanged) ... */
    :root {
        --primary-green: #28a745; --light-gray-bg: #F0F2F6; --card-bg: #FFFFFF;
        --text-color: #212529; --subtle-shadow: 0 4px 6px rgba(0,0,0,0.1);
    }
    .stApp { background-color: var(--light-gray-bg); }
    .card {
        background-color: var(--card-bg); border-radius: 10px; padding: 25px;
        margin-top: 20px; box-shadow: var(--subtle-shadow); color: var(--text-color);
    }
    .stApp h1 {
        font-size: 3rem; font-weight: bold; color: var(--primary-green);
        text-align: center; padding-bottom: 20px;
...
def calculate_team_stats(matches, teams):
    ...
    for m in matches:
        ...
        if g1 > g2:
            stats[t1]['Wins'] += 1; stats[t2]['Losses'] += 1
        elif g2 > g1:
            stats[t2]['Wins'] += 1; stats[t1]['Losses'] += 1
        else:
            stats[t1]['Draws'] += 1; stats[t2]['Draws'] += 1
    for s in stats.values():
        s['GD'] = s['GF'] - s['GA']
        s['Points'] = s['Wins'] * 3 + s['Draws']
    return pd.DataFrame.from_dict(stats, orient='index')
...
with tab1:
    ...
    if not st.session_state.tournament:
        ...
        for i in range(1, num_teams + 1):
            with cols[i-1]:
                available_players = [p for p in st.session_stateAssists': 0}
            stats[p]['Played'] += 1
    ...
    for s in stats.values():
        s['Points'] = s['Wins'] + s['Assists'] + s['Goals'] * 2
    return pd.DataFrame.from_dict(stats, orient='index')
    ...
    if not st.session_state.tournament:
        ...
        for i in range(1, num_teams + 1):
            with cols[i-1]:
                ...
                selected = st..players if p not in assigned_players]
        ...
        col1,multiselect(f"Team {i} Players", options=available_players, key=f"team_{i}")
                if len(selected) > 6:
                    st.warning(f"Team {i} cannot have more than 6 players.")
                    selected = selected[:6]
                team_players[str(i)] = selected
...
with tab2:
    ...
    if not st.session_state.tournament or not st.session_state.tournament.get('history'):
        st.info("At least one match must be played before viewing the summary.")
        st.stop()
    ...
    st.subheader("📊 Team Standings")st.markdown("---")
        
        # --- ROBUST FINISH MATCH LOGIC ---
        if st.button("🏁 Finish & Calculate Match", type="primary", key="finish_match_btn"):
            # Prepare the match data using the current scores from the widgets
            scorers = [event['scorer'] for event in st.session_state.goal_events if event['scorer']]
            assists = [event['assister'] for event in st.session_state.goal_events if event['assister'] and event['assister'] != "-- No Assist --"]
            match = {
                'teams': [t1, t2], 'score': [g1, g2], 'scorers': scorers, 'assists': assists,
            ...
            # Determine winner and next match
            winner = None
            if g1 > g2: winner = t1
            elif g2 > g1: winner = t2
            else:
            ...
            # Reset all temporary states for the new match
            st.session_state.timer_running = False
            st.session_state.timer_start_time = None
            st.session_state.elapsed_time = timedelta(0)
            st.session_state.goal_events = []
            ...
            # Trigger the rerun to refresh the UI for the next match
            st.rerun()
...
with tab2:
    ...
    df_teams_sorted = df_teams.sort_values(by=['Points', 'GD', 'GF'], ascending=False)
    df_players_sorted = df_players.sort_values(by=['Points', 'Goals', 'Assists'], ascending=False)
    ...
    st.markdown("---")
    if st.button("🗑️ Reset Tournament (Current data will be lost)", key="reset_btn"):
