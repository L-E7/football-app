    ...
    if not st.session_state.tournament:
        ...
        if st.button("🚀 התחל טורניר!", key="start_tourney_btn"):
            if team1 == team2:
                ...
                st.session_state.matches = [] # This is for the old logic, can be removed but safe to keep
...
with tab2:
    ...
    st.markdown("---")
    if st.button("🗑️ התחל טורניр חדש (הנתונים הנוכחיים ימחקו)", key="reset_btn"):
        st.session_state.tournament = {}
        st.rerun()
...
with tab3:
    ...
    **ברוכים הבאים למנהל הטורנירים!**
    ...
    - **שלב 3: סיום וסטטיסטיקות**
        ...
        - בסיום הטורניр, תוכל לראות את התוצאות הסופיות.
