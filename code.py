...
# --- NEW: Self-Contained Styling Block (No .toml file needed) ---
# This CSS block replicates the dark theme with green accents directly.
st.markdown("""
<style>
    /* Main App Body */
    .stApp {
    ...
    [data-testid="stSidebar"] {
        background-color: #262730; /* Slightly lighter dark for sidebar */
    }
    ...
    button[kind="primary"]:hover {
        background-color: #256d45; /* A darker green for hover effect */
        color: white;
    }

    /* Metric label styling */
    .st-emotion-cache-1g8sf34 {
        font-size: 1.1rem;
...
def calculate_player_stats(matches):
    ...
    for m in matches:
        match_players = []
        for team in m['teams']:
            match_players.extend(m['players'][team])

        for p in match_players:
            if p not in stats:
                stats[p] = {'משחקים': 0, 'ניצחונות': 0, 'תיקו': 0, 'הפסדים': 0,
                            'שערים': 0, 'בישולים': 0}
            # Only count a "game played" for original team members
            if p in m['original_players'][m['teams'][0]] or p in m['original_players'][m['teams'][1]]:
                 stats[p]['משחקים'] += 1
        ...
        for team_id in m['original_players']:
            for p in m['original_players'][team_id]:
                if winner == team_id:
                    stats[p]['ניצחונות'] += 1
                elif winner is None:
                    stats[p]['תיקו'] += 1
    ...
    if st.button("🚀 התחל טורניר!"):
        if team1 == team2:
            ...
            st.session_state.tournament = {
                ...
                'current_match': [str(team1), str(team2)],
                'history': [],
...
# ---------- Live Match ----------
    ...
    if total_goals > 0:
        ...
        for i in range(total_goals):
            ...
            with col_s:
                st.session_state.goal_events[i]['scorer'] = st.selectbox(
                    f"כובש שער {i+1}", options=all_players_in_match, key=f"scorer_{i}"
                )
            with col_a:
                assist_options = ["-- ללא בישול --"] + all_players_in_match
                st.session_state.goal_events[i]['assister'] = st.selectbox(
                    f"מבשל שער {i+1}", options=assist_options, key=f"assister_{i}"
                )
    ...
    if st.button("🏁 סיים וחשב משחק", type="primary"):
        ...
        # Reset states for the next match, including the fix for goal counters
        st.session_state.timer_running = False
        st.session_state.timer_start_time = None
        st.session_state.elapsed_time = timedelta(0)
        st.session_state.goal_events = []
